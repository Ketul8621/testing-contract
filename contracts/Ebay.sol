...
contract Ebay {
    ...
    function createAuction(string calldata _name, string calldata _description, uint _min) external {
        require(_min > 0, "The minimum value of the auction amount must be more than zero");
    ...
    function createOffer(uint _auctionId) external payable auctionExists(_auctionId)  {
        ...
        Offer storage offer = offers[auction.bestOfferId];

        require(msg.value > auction.min && msg.value > offer.price, "msg.value should be greater than the minimum and best offer till now");
